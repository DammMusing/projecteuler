...
func TestCountCodedTriangleNumbersFromWords(t *testing.T) {
	tests := []struct {
		name string
		want int
	}{
		{name: "answer", want: 162},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := CountCodedTriangleNumbersFromWords(); got != tt.want {
				t.Errorf("CountCodedTriangleNumbersFromWords() = %v, want %v", got, tt.want)
			}
		})
	}
}
